...
BlockCardComponent::BlockCardComponent(MidiBlock* b, int idx)
    : block(b), blockIndex(idx)
{
    jassert(block != nullptr);

    // Power toggle button
    powerButton.setClickingTogglesState(true);
    powerButton.setToggleState(!block->isBypassed(), juce::dontSendNotification);
    powerButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff21262d));
    powerButton.setColour(juce::TextButton::buttonOnColourId, block->getAccentColor().withAlpha(0.85f));
    powerButton.setColour(juce::TextButton::textColourOnId, juce::Colours::black);
    powerButton.setColour(juce::TextButton::textColourOffId, juce::Colour(0xff8b949e));
    powerButton.setTooltip("Enable / Bypass this module");
    ...
    powerButton.setButtonText(!block->isBypassed() ? "ON" : "OFF");
    ...
    // Reorder buttons (quick arrow alternative to drag & drop)
    ...
    removeButton.setColour(juce::TextButton::textColourOffId, juce::Colour(0xfff85149));
    removeButton.setTooltip("Remove this module");
    removeButton.onClick = [this]() { if (onRemove) onRemove(blockIndex); };
    addAndMakeVisible(removeButton);

    // Build controls dynamically
    for (int i = 0; i < block->getNumParameters(); ++i)
    {
        ...
        pc.label->setFont(juce::FontOptions(11.5f, juce::Font::bold));
        pc.label->setColour(juce::Label::textColourId, juce::Colour(0xff8b949e));
        ...
        if (!def.options.empty())
        {
            ...
            for (size_t opt = 0; opt < def.options.size(); ++opt)
                pc.comboBox->addItem(def.options[opt], static_cast<int>(opt + 1));

            int selected = static_cast<int>(std::round(block->getParameterValue(i))) + 1;
            pc.comboBox->setSelectedId(selected, juce::dontSendNotification);
            pc.comboBox->onChange = [this, i, cb = pc.comboBox.get()]() {
                if (block)
                {
                    block->setParameterValue(i, static_cast<float>(cb->getSelectedId() - 1));
                    if (onParameterChanged)
                        onParameterChanged();
                }
            };
        ...
        else
        {
            ...
            pc.slider->setTextBoxStyle(juce::Slider::TextBoxBelow, false, 55, 14);
            ...
            pc.slider->setColour(juce::Slider::thumbColourId, block->getAccentColor());

            pc.slider->onValueChange = [this, i, sl = pc.slider.get()]() {
                if (block)
                {
                    block->setParameterValue(i, static_cast<float>(sl->getValue()));
                    if (onParameterChanged)
                        onParameterChanged();
                }
            };
...
{
    // Start drag and drop when dragging from top header area
...
void BlockCardComponent::timerCallback()
{
    ...
    bool newLed = block->hasActivityAndDecay();
    if (newLed != ledActive)
    {
        ledActive = newLed;
        repaint();
...
void BlockCardComponent::paint(juce::Graphics& g)
{
    ...
    // Subtle outer border, highlighted with accent color if active
    if (!bypassed)
    {
    ...
    g.fillRect(headerRect.removeFromBottom(8.0f)); // flatten bottom curve of header
    ...
    g.drawText(block ? block->getDisplayName() : "Block", 22, 0, 96, 36, juce::Justification::centredLeft, true);
...
void BlockCardComponent::resized()
{
    ...
    right -= 34;
    powerButton.setBounds(right, 7, 32, btnH);

    // Parameters body layout
    int y = 46;
